...
α_init = [0.26 0.7 0.2; 0.18 0.24 0.13]
π_list_init = [0.7; 0.3]

state_list_init = [CTHMM.NormalExpert(0, 1) CTHMM.NormalExpert(0, 1)]

save_name = datadir("cov-2s-3cov-alt")
...
function fit_CTHMM_covariate_model_alt(df, response_list, subject_df, covariate_list, α, π_list, state_list, save_name; kwargs...)
    ...
    with_logger(logger) do
        ...
        try
            result = CTHMM_learn_cov_EM_alt(df, response_list, subject_df, covariate_list, α, π_list, state_list; kwargs...)
...
fit_CTHMM_covariate_model_alt(df_sim, response_list, subject_df, covariate_list, α_init, π_list_init, state_list_init, save_name; 
    max_iter = 2000, α_max_iter = 5, penalty = false)

# CTHMM_learn_cov_EM_alt(df_sim, response_list, subject_df, covariate_list, α_init, π_list_init, state_list_init; 
#     max_iter = 2000, α_max_iter = 5, penalty = false)